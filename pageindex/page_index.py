...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    ...
    while i < len(page_list):
        ...
        if i >= opt.toc_check_page_num and not last_page_is_yes:
            break
        print(f'  checking page {i+1} for TOC...')
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Run checks concurrently with semaphore to limit concurrent API calls
    ...
    tasks = [limited_check(item) for item in indexed_sample_list]
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if mode == 'process_toc_with_page_numbers':
        tracker.set_stage('toc_build', 'with page numbers')
        ...
        tracker.set_stage('toc_build', 'no page numbers')
    ...
    tracker.set_stage('toc_verify', f'{len(toc_with_page_number)} entries')
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        tracker.set_stage('toc_fix', f'{len(incorrect_results)} entries to fix')
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        tracker.set_stage('node_expand', node['title'][:50])
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    tracker.set_stage('node_check', f'{len(toc_with_page_number)} sections')
    ...
    tracker.set_stage('node_expand', f'{len(toc_tree)} top-level sections')
    tasks = [
...
def page_index_main(doc, opt=None):
    ...
    from .progress import tracker
    tracker.set_stage('parse', doc if isinstance(doc, str) else 'PDF')
    print('Parsing PDF...')
    page_list = get_page_tokens(doc, model=opt.model, pdf_parser="PyMuPDF")

    logger.info({'total_page_number': len(page_list)})
    logger.info({'total_token': sum([page[1] for page in page_list])})
    tracker.set_stage('toc_detect', f'{len(page_list)} pages')
    ...
    async def page_index_builder():
        ...
        if opt.if_add_node_summary == 'yes':
            ...
            tracker.set_stage('summarise', f'{node_count} nodes')
...
def page_index(doc, model=None, toc_check_page_num=None, max_page_num_each_node=None, max_token_num_each_node=None,
               if_add_node_id=None, if_add_node_summary=None, if_add_doc_description=None, if_add_node_text=None, pdf_parser="PyMuPDF"):
